                ...
                if 'tcp' in tmp['scan'][host].keys():
                    ...
                    for port in ports:
                        ...
                        if tempportstate.find('open') == -1:
                            self.logger.info("[%s:%s] not open! passing...%s\t%s\t%s\t%s\t%s\t%s\t%s", temphosts,tempport,localtime,tempportstate,tempportname,tempproduct,tempportversion,tempscript,str(tempport))
                            continue
